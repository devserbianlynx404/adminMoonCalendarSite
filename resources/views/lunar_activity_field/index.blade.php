            ...
            <div class="block-header">
                <div class="row">
                    <div class="col-lg-7 col-md-6 col-sm-12">
                        <h2>Список Сфер деятельности</h2>
                            ...
                            <div class="table-responsive">
                                <table class="table table-hover product_item_list c_table theme-color mb-0">
                                    ...
                                    <tr>
                                        ...
                                        <th data-breakpoints="sm xs md">Действие Луны</th>
                                        <th data-breakpoints="sm xs md">Дата изменения</th>
                                        <th>Действие</th>
                                    </tr>
                                    </thead>
                                    <tbody>
                                    @foreach($lunar_day_activity_data as $lunar_day_activity)
                                        <tr>
                                            <td>{{$lunar_day_activity['id']}}</td>
                                            <td>{{\App\Models\LunarDayActivityField::getActivityNameById($lunar_day_activity['field_of_activity_id'])}}</td>
                                            <td>{!! $lunar_day_activity['short_description'] !!}</td>
                                            <td>{{\App\Models\LunarDayActivityField::getNameOfInfluenceById($lunar_day_activity['influence'])}}</td>
                                            <td>{{$lunar_day_activity['updated_at']}}</td>
                                            <td>
                                                <a href="/lunar-activity-field/edit/{{$lunar_day_activity['id']}}" class="js_click_update_day btn btn-default waves-effect waves-float btn-sm waves-green"><i class="zmdi zmdi-edit"></i></a>
